...
충격 가진 후 발생하는 공명 주파수를 고속 푸리에 변환(FFT)으로 추출하여 과실의 비파괴 경도를 분석함
- **주요 학습 스크립트**:
  - `step1_acoustic_fft.py`: 시간 도메인 타격 응답 신호의 FFT 변환, 공명 피크 추출, 질량 보정된 경도 지수 모델링 및 시각화
- ➡️ **[해당 주차 상세 실습 튜토리얼 보기](week13/13주차_실습_음향특성_경도분석.md)**

### [Week 14] 열적 특성 — 냉각 시뮬레이션과 에너지 공학
생물자원의 비열, 열전도율, 열확산율을 이해하고 편미분 방정식 기반의 냉각 공정을 시뮬레이션함
- **주요 학습 스크립트**:
  - `step1_cooling_simulation.py`: 1D 구형 유한차분 해법(solve_ivp) 적용, 중심/표면 온도 시계열 시뮬레이션 및 반감기 산출
...
- **주차별 업데이트 내역**: 
  ...
  - `[Week 13]` FFT 변환을 이용한 음향 공명 주파수 기반 경도 분석 실습
  - `[Week 14]` 유한차분법 기반 1D 구형 냉각 곡선 시뮬레이션 실습
